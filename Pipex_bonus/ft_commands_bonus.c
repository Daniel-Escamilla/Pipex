...
/*   Updated: 2024/02/16 19:20:23 by descamil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_create_path_bonus(char **envp)
{
	char	**path;
	int		i;
	...
	while (envp[++i] != NULL)
	{
		if (ft_strnstr_bonus(envp[i], "PATH=", 5))
			path = ft_split_bonus(envp[i], ':');
	}
...
void	ft_execute_bonus(t_names *names, char **argv, char **envp)
{
	names->route = ft_validate_comm_bonus(names, 1);				// Se llamara como ft_execute(&names, argv, envp);
	names->entire_comm = ft_split_bonus(argv[2], ' ');
